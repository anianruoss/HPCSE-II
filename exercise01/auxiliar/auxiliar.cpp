...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < argc; i++) {
    if (!strcmp(argv[i], "-p"))
      s.problemNumber = atoi(argv[++i]);
    if (!strcmp(argv[i], "-s") || !strcmp(argv[i], "--save")) {
      s.saveOutput = true;
    }
    if (!strcmp(argv[i], "-v") || !strcmp(argv[i], "--verify")) {
      s.checkSolution = true;
    }
  ...
  auto *timePerGrid = (double *)calloc(sizeof(double), s.gridCount);
  auto *timePerOp = (double *)calloc(sizeof(double), 5);
  ...
  for (int i = 0; i < s.gridCount; i++)
    timePerGrid[i] = s.smoothingTime[i] + s.residualTime[i] +
                     s.restrictionTime[i] + s.prolongTime[i] + s.L2NormTime[i];
  for (int i = 0; i < s.gridCount; i++)
    totalSmoothingTime += s.smoothingTime[i];
  for (int i = 0; i < s.gridCount; i++)
    totalResidualTime += s.residualTime[i];
  for (int i = 0; i < s.gridCount; i++)
    totalRestrictionTime += s.restrictionTime[i];
  for (int i = 0; i < s.gridCount; i++)
    totalProlongTime += s.prolongTime[i];
  for (int i = 0; i < s.gridCount; i++)
    totalL2NormTime += s.L2NormTime[i];
  ...
  printf("Smoothing    | ");
  for (int i = 0; i < s.gridCount; i++)
    printf("%6.3f   ", s.smoothingTime[i]);
  printf("|  %6.3f  \n", totalSmoothingTime);
  printf("Residual     | ");
  for (int i = 0; i < s.gridCount; i++)
    printf("%6.3f   ", s.residualTime[i]);
  printf("|  %6.3f  \n", totalResidualTime);
  printf("Restriction  | ");
  for (int i = 0; i < s.gridCount; i++)
    printf("%6.3f   ", s.restrictionTime[i]);
  printf("|  %6.3f  \n", totalRestrictionTime);
  printf("Prolongation | ");
  for (int i = 0; i < s.gridCount; i++)
    printf("%6.3f   ", s.prolongTime[i]);
  printf("|  %6.3f  \n", totalProlongTime);
  printf("L2Norm       | ");
  for (int i = 0; i < s.gridCount; i++)
    printf("%6.3f   ", s.L2NormTime[i]);
  printf("|  %6.3f  \n", totalL2NormTime);
  printf("-------------|-");
  for (int i = 0; i < s.gridCount; i++)
    printf("---------");
