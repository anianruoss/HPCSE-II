...
int main(int argc, char* argv[])
{
	...
	if (rankId == 0) {
		...
		while (!consumers.empty()) {
			...
			if (consumer.future.ready()) {
				...
				if (!samples.empty()) {
					...
					consumerTimes[consumer.rankId - 1] = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();
				}	
		...
		double totalTime = *std::max_element(consumerTimes.begin(), consumerTimes.end());
		double averageTime = std::accumulate(consumerTimes.begin(), consumerTimes.end(), 0.) / nConsumers;
