...
struct Consumer {
    ...
    Consumer(size_t id) : sampleId(id) {
        std::copy(
                &sampleArrayPointer.local()[nParameters*sampleId],
    ...
    static std::tuple<size_t, size_t, double> evaluateSample(Consumer consumer) {
        double fitness = _problem->evaluateSample(consumer.sample);
        return {rankId, consumer.sampleId, fitness};
    }
};


void enqueueFuture(size_t rank, size_t sampleId) {
    futures.push(upcxx::rpc(
                rank, Consumer::evaluateSample, Consumer{sampleId}
                ).then([](std::tuple<size_t, size_t, double> result) {
                    _solver->updateEvaluation(std::get<1>(result), std::get<2>(result));
