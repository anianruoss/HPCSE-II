...
int main(int argc, char *argv[]) {
  ...
  if (X1.rankId == MPI_PROC_NULL)
    for (int i = 0; i < fy; i++)
      for (int j = 0; j < fz; j++)
        U[j * fx * fy + i * fx + (nx + 1)] = 0;
  if (Y1.rankId == MPI_PROC_NULL)
    for (int i = 0; i < fx; i++)
      for (int j = 0; j < fz; j++)
        U[j * fx * fy + (ny + 1) * fx + i] = 0;
  if (Z1.rankId == MPI_PROC_NULL)
    for (int i = 0; i < fx; i++)
      for (int j = 0; j < fy; j++)
        U[(nz + 1) * fx * fy + j * fx + i] = 0;
...
#pragma omp parallel for collapse(3)
    ...
    t_end = MPI_Wtime();
    computeTime += double(t_end - t_init);
    t_init = MPI_Wtime();

    MPI_Irecv(&U[0], 1, faceXType, X0.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Irecv(&U[(nx + 1)], 1, faceXType, X1.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Irecv(&U[0], 1, faceYType, Y0.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Irecv(&U[fx * (ny + 1)], 1, faceYType, Y1.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Irecv(&U[0], 1, faceZType, Z0.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Irecv(&U[fx * fy * (nz + 1)], 1, faceZType, Z1.rankId, 1, gridComm,
              &request[request_count++]);

    t_end = MPI_Wtime();
    recvTime += double(t_end - t_init);
    t_init = MPI_Wtime();

    MPI_Isend(&U[1], 1, faceXType, X0.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Isend(&U[nx], 1, faceXType, X1.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Isend(&U[fx], 1, faceYType, Y0.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Isend(&U[fx * ny], 1, faceYType, Y1.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Isend(&U[fx * fy], 1, faceZType, Z0.rankId, 1, gridComm,
              &request[request_count++]);
    MPI_Isend(&U[fx * fy * nz], 1, faceZType, Z1.rankId, 1, gridComm,
              &request[request_count++]);

    t_end = MPI_Wtime();
    sendTime += double(t_end - t_init);
    t_init = MPI_Wtime();

    MPI_Waitall(request_count, request, MPI_STATUS_IGNORE);

    t_end = MPI_Wtime();
    waitTime += double(t_end - t_init);
    t_init = MPI_Wtime();
  }
  ...
  MPI_Reduce(&computeTime, &sumComputeTime, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&packTime, &sumPackTime, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&unpackTime, &sumUnpackTime, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&sendTime, &sumSendTime, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&recvTime, &sumRecvTime, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&waitTime, &sumWaitTime, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  ...
  double totalTime = meanComputeTime + meanRecvTime + meanPackTime +
                     meanSendTime + meanWaitTime + meanUnpackTime;
